      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-8">
            <div>
              <p className="landing-section-label">Gentle momentum</p>
            ...
            <div className="flex items-center gap-4 shrink-0">
              <motion.div
                ...
                className="landing-streak-flame w-16 h-16 rounded-2xl bg-gradient-to-br from-cyan-500/20 to-cyan-600/10 flex items-center justify-center border border-cyan-500/20"
              >
                <Flame className="w-8 h-8 text-cyan-500" fill="currentColor" fillOpacity={0.35} />
              </motion.div>
              <div>
                <motion.p
                  initial={{ opacity: 0 }}
          ...
          <div className="mt-10 flex flex-wrap gap-2 justify-center sm:justify-start">
            {Array.from({ length: STREAK_DAYS }).map((_, i) => (
              <motion.div
                ...
                transition={{ delay: 0.03 * i, duration: 0.3, ease: [0.34, 1.56, 0.64, 1] }}
                className={`w-3 h-8 sm:w-3.5 sm:h-9 rounded-full ${
                  i >= STREAK_DAYS - 3
                    ? 'bg-gradient-to-t from-cyan-600/80 to-cyan-400/60 shadow-[0_0_12px_rgba(14,116,144,0.35)]'
                    : i >= STREAK_DAYS - 7
                      ? 'bg-cyan-600/50'
                      : 'bg-[var(--border-light)]'
                }`}
